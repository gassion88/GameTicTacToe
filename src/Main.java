...
public class Main {
    ...
    private static String GAME_STATE_X_WIN = "X выграл !";
    private static String GAME_STATE_O_WIN = "O выграл !";
    private static String GAME_STATE_DRAY = "Ничья !";
    private static String GAME_STATE_NOT_FINISHED = "Игра закончена !";
    ...
    public static int[] inputCellCoordinates(String[][] board) {
        ...
        do{
            System.out.println("Введите коодинаты через пробел (0-2)");
            ...
            if ((row < 0) || (row >= ROW_COUNT) || (col < 0) || (col >= COL_COUNT) ) {
                System.out.println("Ошибка, ведите от 0 до 2");
    ...
    public static String checkGameState( String[][] board ){
        ...
        int leftDiagonalSun = 0;
        for (int i = 0; i < ROW_COUNT; i++){
            leftDiagonalSun += calculateNumValue(board[i][i]);
        }
        sums.add(leftDiagonalSun);

        int rightDiagonalSun = 0;
        for (int i = ROW_COUNT-1; i >= 0; i--){
            rightDiagonalSun += calculateNumValue(board[i][i]);
        }
        sums.add(rightDiagonalSun);

        if (sums.contains(3)){
            return GAME_STATE_X_WIN;
        } else if (sums.contains(-3)) {
            return GAME_STATE_O_WIN;
        } else if (arrAllCellsTaken(board)) {
            return GAME_STATE_DRAY;
        }else {
    ...
    public static boolean arrAllCellsTaken( String[][] board){
        for (int row = 0; row < ROW_COUNT; row++) {
            for (int col = 0; col < COL_COUNT; col++) {
                if (Objects.equals( board[row][col], CELL_STATE_EMPTY)){
                    return false;
